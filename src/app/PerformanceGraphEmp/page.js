...
const EmployeeWorkStatus = ({ employeeID, employeeName, employeeTasks }) => {
...
const App = () => {
  ...
  // Filter the employee data based on the entered filter code
  useEffect(() => {
  ...
  }, [filterCode, startDate, endDate, employeeData]);
    ...
    <div className="container mx-auto p-6">
      ...
      {uniqueEmployeeIDs.length === 0 ? (
        ...
        uniqueEmployeeIDs.map((employeeID) => {
          ...
          return <EmployeeWorkStatus key={employeeID} employeeID={employeeID} employeeName={employeeName} employeeTasks={employeeTasks} />;
        })
      )}
    </div>
  );
